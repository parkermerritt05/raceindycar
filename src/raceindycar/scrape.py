...
SESSION_PICKLE = "session.pkl"


def load_race(race_id, session_id=None, laps=False):
    ...
    if parsed is not None and (not laps or "laps" in parsed):
    ...
    if laps:
        ...
        payload["laps"] = build_lap_rows(positions, metrics, names, teams)
...
def build_lap_rows(positions, metrics, names, teams):
    ...
    for car, lap in sorted(set(positions) | set(metrics)):
        ...
        row.update(extra.get("sections") or {})
        row.update(extra.get("pits") or {})
        rows.append(row)
    return rows
